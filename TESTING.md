...
- [ ] **AirPods exclusive device capture (Proton Meet hijack)** — During a Proton Meet call, AirPods are captured as input (priority input). Screenpipe's CoreAudio client gets zero-filled buffers. Watchdog waits 30s from stream startup (grace period), then detects sustained zero-fill and rebuilds the device. Audio resumes when Proton Meet releases exclusive access. Verify: health endpoint / tray shows connection re-established within 35s; logs show "hijack: last_non_zero_at exceeded timeout" and device rebuild. (`a2e89b2ae`)
- [ ] **USB device zero-fill false-positive prevention** — Attach USB hub or inactive USB mic (never produced real audio). Verify watchdog does NOT fire repeatedly. Verify: logs show device startup but no rebuild loop after 30s. The grace period (only trigger watchdog after first real audio) prevents recovery storms on devices that deliver sustained zeros from the start. (`357e4dfcc`)
