...
- **authoritative active-time total in activity summary** — the activity summary API now returns a single, deterministic `total_active_minutes` for the whole range alongside the per-app/window breakdown, so dashboards and agents no longer undercount by summing the (capped) top-20 windows. see [api recipes](/api-recipes).
- **redact IDs, URLs, and dates from screen data** — the **settings → privacy → AI PII removal** field selector now includes IDs (SSNs, credit cards, account numbers), URLs, and Dates as opt-in categories on top of the existing names/emails/phones/addresses toggles. secrets stay always-on. drives both the text and image redactors. see [privacy filter](/privacy-filter).
- **disable timeline toggle** — a new **settings → display → disable timeline** switch turns the timeline off end-to-end: it disappears from the sidebar, the shortcut-reminder overlay stops surfacing, and timeline-only backend work (hot-cache warm-up, frame/audio buffering) is skipped on restart. see [search screen history](/search-screen-history).
- **calendar connections shown from meeting notes** — opening calendars from a meeting note now surfaces your connected calendars instead of an empty picker. see [connections](/connections).

### updates

- **stronger image PII model (rfdetr_v11)** — the on-device image PII redactor shipped a new 512×512 model. on real screen frames detection rate goes from ~7% (v9) to 100% at the production threshold, and the synthetic benchmark hits 98.9% zero-leak with 0% oversmash. existing installs keep running the previous model until they update. see [privacy filter](/privacy-filter).
- **owned-browser cookie consent is now a single, persistent choice** — the owned browser asks once for browser-session cookie access on macOS, persists the answer (undecided / disabled / enabled), and never re-prompts per domain. a new cookie menu next to refresh shows status and lets you enable, revoke, or retry. macOS preflights the Keychain ACL so you only see a warning when the OS would actually prompt.
- **`auto` (Gemini) stops misfiring web search on your local data** — the AI gateway no longer hijacks "search the web" phrasing, no longer runs web_search inline against your private localhost API, and the tool is scoped to public info only — so Gemini Flash stops dumping raw grounding results into chat after a local-API hiccup.

### bug fixes

- **no more duplicate ongoing meetings** — a database-level invariant now enforces "at most one open meeting" and heals any pre-existing duplicates on upgrade, so parallel start events can't fork a meeting into two parallel rows. see [meeting intelligence](/meeting-intelligence).
- **image PII redaction actually runs on v2.4.305+** — the redactor was silently failing every inference because the input-size detector didn't recognize the new 512×512 model and fell back to 384×384. image redaction is now wired correctly. see [privacy filter](/privacy-filter).
- **fewer crashes on fresh installs and chat requests with no model** — the database now creates its parent directory before opening (kills the top "unable to open database file" crash on fresh installs and relocated volumes), and the AI gateway no longer throws when a chat request omits the model field.
- **Apple OCR bounding boxes restored** — macOS OCR results once again include the per-word bounding boxes that the timeline and search overlays use to highlight matches.
- **timeline Live Text overlay stops blocking chat input on macOS** — the native VisionKit overlay is hidden when the timeline loses focus, so it can't sit on top of the chat composer and silently eat keystrokes.
- **owned browser stays hidden when navigated in the background** — agents and pipes navigating the owned browser no longer pop it over your meeting notes, timeline, or settings; the page still loads in the background and appears the next time you open the chat sidebar.
- **meeting note editor reachable at the bottom** — the meeting note's lower lines used to sit behind the docked footer and only the arrow keys could reach them. the footer is now a non-overlapping flex child, so clicking the last lines places the caret correctly. see [meeting intelligence](/meeting-intelligence).
- **chat composer drafts no longer leak across conversations** — each conversation now keeps its own composer draft instead of sharing one across the sidebar.
- **no more duplicate sidebar chats from parallel auto-sent prefills** — two windows firing the same auto-sent prompt now deterministically pick a single winner, so you get one conversation in the sidebar instead of two.
- **no stray text selection in empty chat areas** — click-dragging across the empty welcome area no longer paints a phantom selection highlight on macOS.
- **pipe notification toggles work on CRLF-authored pipes** — pipes whose `pipe.md` was saved with Windows line endings now show the correct notification-bell state, and disabling notifications no longer prepends a duplicate frontmatter block. see [pipes](/pipes).
- **onboarding respects "skip" with zero pipes selected** — finishing onboarding with no pipes selected no longer silently re-enables the defaults. see [getting started](/getting-started).
- **"check for updates" toast is honest** — clicking check-now when an update is already pending now surfaces that state instead of a generic toast.
- **pipe preset IDs trimmed during parsing** — whitespace around preset IDs in pipe configs no longer breaks lookup. see [pipe store](/pipe-store).
