...
- **chat history page** — a dedicated chats view with search, bulk select, archive, and a sidebar entry. the list paginates 30 at a time so chat history stays fast even with hundreds of conversations.
- **codex-style follow-up and steering** — queue follow-ups and steer the current run from a redesigned chat composer, including uploads on queued messages and a cleaner stop/queue UI.
- **smart connection suggestions on the chat dashboard** — the home page now surfaces relevant integrations to connect based on what you're typing, with one-click jumps into the connections panel.
- **drag-to-merge meetings** — drag one meeting onto another to merge them in the meetings list. titles, attendees, and notes from both sides are preserved.
- **resizable left sidebar** — drag the hairline handle to set your preferred sidebar width; the size is remembered across launches.
- **collapsible transcript search + caret-follow editor** — the meeting transcript search panel now collapses out of the way, and the note editor scrolls to keep the caret in view while you type. see [meeting transcription](/meeting-transcription).
- **meeting note typing toggle** — new setting to stop screenpipe from auto-appending whatever you type elsewhere into the active meeting note. see [meeting transcription](/meeting-transcription).
- **disable the live-meeting notification** — settings toggle to silence the "meeting detected" toast if you'd rather start notes manually.
- **Google OAuth unverified-app walkthrough** — built-in guide that explains the unverified-app warning and walks you through approval when connecting Google services. see [connections](/connections).
- **filter and sort connections** — the connections page now has filter and sort controls so finding a specific integration is faster as your list grows. see [connections](/connections).
- **`screenpipe survey` CLI** — quick command to open the screenpipe user survey from the terminal. see the [CLI reference](/cli-reference).
- **Windows input-latency mode** — opt-in `--prioritize-input-latency` flag (with `--extraction-thread-priority` and `--pause-extraction-on-input-ms` tunables) reduces mouse and keyboard lag during long Windows accessibility capture sessions. see the [CLI reference](/cli-reference).
- **event-driven capture interval overrides** — four new CLI flags (`--idle-capture-interval-ms`, `--visual-check-interval-ms`, `--visual-change-threshold`, `--min-capture-interval-ms`) let you tune individual capture intervals without changing power mode — useful for always-on desktop and task-mining setups. see the [CLI reference](/cli-reference).
- **`/activity-summary` covers agent context** — the existing summary endpoint now returns recording health, memories, screen/audio snippets, and guidance behind `include_*` flags, plus `data_status` and `query_status` so agents can tell "not recording" apart from "no matches". the separate agent-context route is retired. see the [API docs](/for-developers).
- **`/health` reports clipboard and UI capture status** — the health endpoint now exposes whether the UI recorder is configured, running, and capturing clipboard events, so monitoring tools can answer "is clipboard capture on?" without database queries. see the [API docs](/for-developers).
- **Pro users default to Screenpipe Cloud transcription** — signed-in Pro accounts now use Screenpipe Cloud (Deepgram) for audio out of the box, with live meeting transcription on by default. explicit engine choices are still respected.

### updates

- **chat composer polish** — tighter spacing, cleaner suggestion rows, and a fallback sparkle icon when a model-specific icon is missing.
- **chat suggestions and sources** — the sources panel is deferred while a response is streaming, and the empty state hides while a pipe is still producing output.
- **edit chat user messages** — improved UX for editing and re-running your own messages mid-conversation.
- **chat-sidebar pipe rows appear instantly** — pipe-run entries show up the moment they finish instead of waiting for the next refresh.
- **calendar handling for live meetings** — Google Calendar events are deduped across accounts, the calendar refresh runs ahead of meeting-detection notifications, and Google Calendar OAuth reconnects no longer fail. the nearby-event join CTA is softer and less aggressive.
- **live transcript reliability** — auto-detected meetings now show a live transcript, reconnects after a stream drop are faster, and stall notifications are passive instead of blocking. see [meeting transcription](/meeting-transcription).
- **meeting-notes consent and search** — transcript search is now inline in the notes view, and the consent line moved to the footer where it belongs.
- **meeting-detected notification** — collapsed to a single primary action so it's easier to dismiss or act on.
- **embedded browser stays in its lane** — the native webview hides when you close the panel or navigate away, bare domains like `youtube.com` work without typing `https://`, and schemes like `about:`, `data:`, and `file:` are preserved correctly.
- **browser session reuse on Windows** — the embedded browser now reuses your existing browser session on Windows, and the extension popup is smarter about pairing state.
- **Deepgram transcription tuning** — batch size and worker timeout are aligned so cloud transcription keeps up with longer recordings.
- **transcript engine dropdown cleanup** — the standalone "openai realtime" option has been removed; configure realtime via your AI preset instead.
- **`screenpipe status` reads the right database** — the CLI now resolves `db.sqlite` from your configured base directory instead of assuming `./data/`. see the [CLI reference](/cli-reference).
- **`/health` can't be deadlocked** — the health endpoint now bounds its own response time so external watchdogs can't kill the CLI when a subsystem stalls. see the [API docs](/for-developers).
- **ffmpeg ships self-contained on macOS** — the bundled ffmpeg binaries are preferred over anything on `PATH`, so a stray Homebrew install can't break recording.
- **drag-and-drop image uploads** — chat now accepts images dropped from anywhere on your home directory, not just a hard-coded path.

### bug fixes

- **audio reconciliation loop** — fixed a "zombie" state where audio chunks could be retried forever; chunk processing now tracks an explicit completion state.
- **Screenpipe Cloud chat image forwarding** — images attached to chat messages now reach the cloud model instead of being dropped.
- **reopened chats render tool calls** — inline tool-call XML is rewritten on load so historical chats no longer show empty tool rails.
- **chat blank after meeting Summarize** — pressing Summarize in a meeting note no longer leaves the resulting chat blank.
- **phantom "new content" pill in chat** — the unread pill now clears cleanly once the layout settles.
- **AI preset duplicates** — the model picker no longer shows the same model twice when the API returns duplicates.
- **meeting "join and take notes"** — clicking the notification now opens the correct live note, and silence-only stall warnings are suppressed.
- **speaker delete works again** — deleting a speaker no longer silently fails when audio chunk files are missing on disk.
- **chat empty state during pipe streaming** — the empty-state placeholder no longer flashes while a pipe is producing output.
- **tray status on permission denied** — the menu-bar icon now flips to Error immediately when recording is blocked by permissions, instead of staying green.
- **`pipe-watch` and pipe spinner overlay** — the running-pipe spinner hides correctly when the sidebar is collapsed.
- **owned-browser drag pill** — clicking the floating overlay no longer fires its action when you were just dragging it.
- **Windows mic transcription** — `INSERT OR IGNORE` on transcription writes prevents duplicate-key churn under heavy load.
- **OAuth error visibility** — unreadable OAuth tokens now surface a clear state and decryption failures are logged instead of failing silently.
- **focus monitor identity** — focused-monitor tracking now matches displays correctly when ports or names change.
- **PII redactors get the cloud token** — the Tinfoil text and image redactors now receive the cloud auth token, so cloud-assisted redaction actually runs when enabled. see [privacy filter](/privacy-filter).
- **connections panel input** — typing in the connection search/filter input works again.
