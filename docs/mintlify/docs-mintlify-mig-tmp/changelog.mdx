...
- **Windows microphone acoustic echo cancellation** — Windows builds now wire the system AEC pipeline into screenpipe's audio capture, so meeting mics on Windows pick up far less of the other side's voice bleeding back through speakers. see [meeting transcription](/meeting-transcription).
- **source citations in chat** — answers in chat now include a citation footer that points to the underlying captured screens, audio, or memories the response was grounded in.

### updates

- **faster app launch** — heavy renderer panels (timeline, meetings, chat) now defer their initial render so the main window paints sooner on cold start.
- **polished live meeting transcript UI** — tightened spacing, button styling, and live updating in the meeting notes panel. see [meeting transcription](/meeting-transcription).

### bug fixes

- **Screenpipe Cloud transcription applies without a full restart** — switching to or from Deepgram-backed Screenpipe Cloud transcription now propagates through the audio manager and meeting detector immediately, instead of requiring a full server restart to take effect. see [meeting transcription](/meeting-transcription).
- **meeting transcript no longer drops when live STT fails** — transient live transcription failures during a meeting no longer wipe captured segments; the saved transcript stays intact even when the streaming engine errors out.
