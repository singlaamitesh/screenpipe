...
- **annual Pro plan** — in-app upgrade from **settings → account** now defaults to annual ($500/yr, shown as $42/mo billed annually, save $100) with an inline toggle back to $50/mo. checkout pins your screenpipe account so a different email at the payment step can't strand the subscription. see [teams](/teams).
- **drag-and-drop images into meeting notes** — drag image files straight onto an open meeting note to insert them at the drop point. the old toolbar image button is gone; non-image drops show a toast instead of failing silently. see [meeting transcription](/meeting-transcription).
- **device skills connection** — a new "skills" card in **settings → connections** scans your device for `SKILL.md` folders (like `~/.claude/skills` or any folder you pick) and imports the ones you choose. imported skills are mirrored into every pipe and chat session automatically, so the agent picks them up everywhere with no per-pipe wiring. see [connections](/connections).
- **cross-device connection sync** — opt in from **settings → account** to sync your manual and OAuth connection credentials across machines signed into the same screenpipe account. credentials are end-to-end encrypted; OAuth refreshes are rotation-safe, so a stale token on one device can't clobber a freshly rotated one on another and brick the connection. off by default. see [connections](/connections).
- **installed-but-uncaptured apps in privacy filters** — the ignored/included app filters now autocomplete every app installed on your machine, not just apps that already have frames in your timeline. each uncaptured app shows its real icon and an "installed · not captured yet" hint, so you can pre-emptively exclude an app before it ever gets recorded. see [privacy filter](/privacy-filter).
- **work-hours schedule from the CLI** — `screenpipe record` accepts `--schedule-enabled` and repeatable `--schedule-rule "day,start,end,mode"` flags (mode = `all` / `audio_only` / `screen_only`), so embedders and CLI users can drive the work-hours schedule without hand-editing config. capture pauses and resumes in-process — no restart. see the [CLI reference](/cli-reference).
- **ICS calendar connection** — a new agent-facing ICS calendar connection lets you point screenpipe at any iCalendar URL and pull events into chat and pipes alongside Google Calendar and Microsoft 365. see [connections](/connections).
- **Cmd+Enter steers queued chat prompts** — Cmd+Enter (Ctrl+Enter on Windows/Linux) on a queued chat follow-up sends it as a native steer to the running agent, and a fresh native steer preempts anything still in the queue.
- **minimize-to-tray toggle on Windows** — a new switch in **settings → general** keeps screenpipe alive in the system tray when you close the main window on Windows, matching the macOS behavior. closing the window when the toggle is off still quits the app.

### updates

- **app access gated by an active subscription** — the desktop app now requires an active screenpipe subscription (Standard, Pro, or Enterprise) to launch into the main UI. lifetime customers are grandfathered, recent purchases unlock immediately via a one-shot server check (no waiting on webhooks), and the gate stays valid through stale-cache and offline windows so a local-first app can't lock you out when the server is unreachable. signing in or paying mid-session restarts capture automatically. see [teams](/teams).
- **pipe installs use a tier-safe AI preset** — pipe installation and execution now run through the `pipes` preset (auto-routed) instead of the Opus chat default, and the gateway validates the chosen model against your plan's tier list with a graceful fallback so a pipe install can't fail on a model you don't have access to. premium models you've explicitly chosen are preserved even when the gateway briefly returns a fallback catalog. see [pipes](/pipes).
- **engine writes `last-panic.log` and flushes crash reports on panic** — the `screenpipe` CLI/engine binary now writes a fsync'd `last-panic.log` (with the previous one rotated to `.prev`) and flushes Sentry on Rust panics, so hosts that embed screenpipe as a child process can read the cause after the process exits. written regardless of telemetry setting, next to `screenpipe.log` in your data dir, and stamped with embedder attribution. see the [CLI reference](/cli-reference).
- **Windows CLI is code-signed and no longer false-kills inside embedded hosts** — the `screenpipe` Windows CLI binary shipped via npm is now signed with the SSL.com EV cert, so corporate Windows no longer flags it as "unknown publisher" and Defender/EDR no longer scrutinizes it on launch. the `--auto-destruct-pid` watcher also no longer requires `screenpipe-app.exe` to be running, so Electron and other non-Tauri hosts can embed the CLI without it self-terminating within seconds. VC++ runtime DLL bundling now hard-fails on the build side instead of shipping a package that dies with `STATUS_DLL_INIT_FAILED`. see the [CLI reference](/cli-reference).
- **clearer keyboard shortcut labels** — shortcut labels across the reminder overlay, tray menu, timeline controls, sidebar search, and chat UI now stay in sync with your current settings, and a shortcut that's disabled or unset is hidden instead of showing a dead binding. the tray menu refreshes immediately after you change a shortcut.
- **"clear cache" button labelled "clear"** — the button under the Clear Cache card in settings now reads "clear" instead of the confusing "scan".

### bug fixes

- **Apple Calendar permission flow restored on macOS** — granting Apple Calendar access from **settings → connections** works again. the signed production bundle was missing the calendar entitlement, so macOS denied EventKit before the permission prompt could fire and screenpipe never appeared under **System Settings → Privacy & Security → Calendars**. see [connections](/connections) and [permissions](/permissions).
- **timeline memory markers land on the right frame** — the diamond markers on the timeline were positioned left-to-right while the frame row renders right-to-left, so the whole memory layer was rendering mirrored (and shifted further by day-boundary dividers in multi-day views). markers now snap to the nearest frame by real position. see [AI memory](/ai-memory).
- **recording resumes on macOS after signing back in** — after signing out and signing back in on macOS, recording stayed stopped because the resume effect only matched the Windows `main`/`main-window` label. the owner-window check is now platform-aware (`home` on macOS) so capture restarts the moment access flips back to entitled.
- **logout no longer needs two clicks** — an in-flight background refresh could resurrect a just-cleared session a beat after you clicked logout, so you had to click again. signing out now bumps a monotonic auth generation that stale refreshes check before writing, so a logout always sticks on the first click.
- **Pro access kept until your billing period ends after cancellation** — canceling a Pro subscription no longer immediately revokes cloud-AI access. as long as your paid period hasn't ended yet, Pro features stay on through the end of the cycle. dunning states (past_due / unpaid / incomplete) still gate access as before.
- **entitlement-resume restart can't wedge capture anymore** — the resume effect that restarts capture on a sign-in or purchase now runs only once, from a single window, with a proper stop → settle → spawn sequence. a quick disconnect-then-reconnect no longer races the in-flight teardown and leaves the recorder stuck on "Starting capture session".
- **sign-in always completes on the gate screen** — the deep-link handler that catches the `screenpipe://` OAuth callback is now mounted outside the subscription gate, so first-launch sign-in completes for users who land on the "sign in required" screen. previously the handler was unmounted with the gated UI and the login token was dropped.
- **"what's new" toast respects the right notification toggle** — the auto-update "what's new" toast is now tagged as an app-update notification, so turning off pipe notifications no longer suppresses release notes.
- **`/notify` honors the pipe notifications toggle** — pipe-driven notifications posted to the local `/notify` endpoint now check `notificationPrefs.pipeNotifications` before showing or persisting, matching how display-change notifications behave. system notifications are unaffected.
- **chat handles large pasted context cleanly** — pasting a very large block of text into the chat composer no longer hangs or corrupts the conversation.
- **scheduled pipes honor your local timezone** — raw cron schedules (e.g. `0 7 * * *`) now fire at 07:00 local time instead of 07:00 UTC. the "every day at Nam" UI path is consistent across DST. see [pipes](/pipes).
- **AI gateway no longer crashes on malformed requests** — null-safety guards in the gateway's provider and prompt-hint paths turn malformed requests (missing model, missing messages) into clear errors instead of crashing the worker.
- **pinned and hidden chats survive session save** — chats you've pinned or hidden no longer revert when conversations are persisted.
- **quieter macOS audio logs** — silenced a recurring ScreenCaptureKit audio-drop log on macOS so the engine log file is easier to skim in your feedback bundle.
- **annual upgrade copy fixed** — the in-app annual upgrade card now reads "save $100" (the real savings vs. $50/mo) instead of "save $200".
